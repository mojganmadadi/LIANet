...
def main_cfg(args: DictConfig):
    ...
    from datasets import DynamicWorld, MetaCanopyHeights, DominantLeafTypeSegmentation, BuildingCoverageRaster, BuildingBinaryRaster, PASTIS
    ...
    print(f"Number of training samples: {len(train_ds)} from folds {[f for f in range(5) if f not in val_folds]}")
    print(f"Number of validation samples: {len(val_ds)} from folds {val_folds}")
    ...
    if args.model_type in ["replace_final_block"]:
        model = DownstreamModel(
            ...
            checkpoint_path_relative="model_checkpoints/checkpoint_Epoch300_Iteration600000.pt",
    ...
    def forward_model(model, model_type, batch):
        ...
        # OR: our fancy super cool model witch is way better :)
            ...
            reconstruction, outputs = model(timestamp, y_s2, x_s2)
